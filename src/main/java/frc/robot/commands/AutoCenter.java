...
public class AutoCenter extends CommandBase {
  ...
  public AutoCenter(Swerve s_Swerve, PhotonVisionWrapper s_PhotonVisionWrapper) {
    ...
    pidControllerDriveX = new PIDController(Constants.Swerve.driveKP, Constants.Swerve.driveKI,
        Constants.Swerve.driveKD);
    pidControllerDriveX.setTolerance(.15);
    pidControllerDriveX.setSetpoint(1.2);

    pidControllerDriveY = new PIDController(Constants.Swerve.driveKP, Constants.Swerve.driveKI,
        Constants.Swerve.driveKD);
    pidControllerDriveY.setTolerance(.3);
    pidControllerDriveY.setSetpoint(0);

    pidControllerAngle = new PIDController(Constants.Swerve.angleKP, Constants.Swerve.angleKI,
   ...
   * It drives the robot a set distance away from the april tag by finding the
   * translation values
   * for the x and y distances, and the angle from the april tag to create a
   * 2d translation for the swerve subsystem. The translation value for x is the
   * frward distance from the
   * limelight to the april tag and the y distance is the left value. The x
   * translation value is -1 so that the robot
  ...
  public void execute() {
    ...
    if (target != null) {
      ...
      double translationValX = MathUtil.clamp(pidControllerDriveX.calculate(targetLocation.getX()),
          -Constants.Swerve.autoCenterLimit, Constants.Swerve.autoCenterLimit);
      double translationValY = MathUtil.clamp(pidControllerDriveY.calculate(targetLocation.getY()),
          -Constants.Swerve.autoCenterLimit, Constants.Swerve.autoCenterLimit);
