    ...
    public void execute() {
        for (int i = 0; i < m_ledBuffer.getLength() + LEDConstants.OrangeDot.pauseBetween; i++) {
            int value = MathUtil.clamp(
                    (int) ((1 / LEDConstants.OrangeDot.spread) * (Math.abs(m_orangeDotMiddleIndex - i)) * hsv.v)
        ...
        m_orangeDotMiddleIndex = (m_orangeDotMiddleIndex + LEDConstants.OrangeDot.speed) > m_ledBuffer.getLength()
                + LEDConstants.OrangeDot.pauseBetween ? -LEDConstants.OrangeDot.pauseBetween
